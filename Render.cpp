...
bool compileShaders()
{
	...
	res &= shaders.insert(std::pair<std::string, Shader>("basicMaterial", Shader("basicMaterial.vs", "basicMaterial.fs"))).second;
	res &= shaders.insert(std::pair<std::string, Shader>("texturedMaterial", Shader("texturedMaterial.vs", "texturedMaterial.fs"))).second;
